...
const getDiffString = (amount: number): string =>
  amount == 0
    ? ""
    : amount > 0
    ? `⬆️ ${Number(amount.toFixed(2)).toLocaleString()}`
    : `⬇️ ${Number(Math.abs(amount).toFixed(2)).toLocaleString()}`;
  ...
  return (
    <>
      ...
      <li>
        ...
        {data.recovered_diff != 0 && `(${getDiffString(data.recovered_diff)})`}
      </li>
      <li>
        {data.active_cases.toLocaleString()} Active Cases{" "}
        {data.new_cases - data.recovered_diff != 0 &&
          `(${getDiffString(data.new_cases - data.recovered_diff)})`}
      </li>
      <li>{data.new_cases.toLocaleString()} New Cases</li>
      <li>
        {data?.swab_count?.toLocaleString() || "N/A"} Swabs{" "}
        {data?.swab_count != 0 && `(${getDiffString(data.swab_count_diff)})`}
      </li>
      <li>
        {data?.positivity_rate?.toFixed(1) || "N/A"}% Positivity Rate{" "}
        {data?.positivity_rate_diff != 0 &&
          `(${getDiffString(data.positivity_rate_diff)}%)`}
      </li>
      <li>
      ...
      <li>
        {data.received_first_booster_dose
          ...
          : "N/A"}{" "}
        First Booster Doses{" "}
        {data?.received_first_booster_dose_diff > 0 &&
          `(⬆️ ${data?.received_first_booster_dose_diff.toLocaleString()})`}
      </li>
      <li>
        {data.received_second_booster_dose
          ? data.received_second_booster_dose.toLocaleString()
          : "N/A"}{" "}
        Second Booster Doses{" "}
        {data?.received_second_booster_dose_diff > 0 &&
          `(⬆️ ${data?.received_second_booster_dose_diff.toLocaleString()})`}
      </li>
